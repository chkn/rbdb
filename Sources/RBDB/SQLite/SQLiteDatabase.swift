...
public class SQLiteDatabase {
	...
	public func query(_ sql: String, parameters: [Any?]) throws -> [[String: Any?]] {
		return try sql.withCString { sqlCString in
			...
			guard sqlite3_prepare_v2(db, sqlCString, -1, &statement, nil) == SQLITE_OK else {
				let errmsg = String(cString: sqlite3_errmsg(db))
				throw SQLiteError.queryError(errmsg)
			}
			defer {
			...
			guard parameters.count == expectedParams else {
				throw SQLiteError.queryParameterCount(expected: expectedParams, got: parameters.count)
			}
			...
			for (index, parameter) in parameters.enumerated() {
				...
				case let stringValue as String:
					guard sqlite3_bind_text(statement, paramIndex, stringValue, -1, unsafeBitCast(-1, to: sqlite3_destructor_type.self)) == SQLITE_OK else {
						let errmsg = String(cString: sqlite3_errmsg(db))
						throw SQLiteError.queryError("Failed to bind string parameter at index \(index): \(errmsg)")
					}
				case let intValue as Int:
					guard sqlite3_bind_int64(statement, paramIndex, Int64(intValue)) == SQLITE_OK else {
						let errmsg = String(cString: sqlite3_errmsg(db))
						throw SQLiteError.queryError("Failed to bind int parameter at index \(index): \(errmsg)")
					}
				case let int64Value as Int64:
					guard sqlite3_bind_int64(statement, paramIndex, int64Value) == SQLITE_OK else {
						let errmsg = String(cString: sqlite3_errmsg(db))
						throw SQLiteError.queryError("Failed to bind int64 parameter at index \(index): \(errmsg)")
					}
				case let doubleValue as Double:
					guard sqlite3_bind_double(statement, paramIndex, doubleValue) == SQLITE_OK else {
						let errmsg = String(cString: sqlite3_errmsg(db))
						throw SQLiteError.queryError("Failed to bind double parameter at index \(index): \(errmsg)")
					}
				case let dataValue as Data:
					let result = dataValue.withUnsafeBytes { bytes in
						sqlite3_bind_blob(statement, paramIndex, bytes.baseAddress, Int32(dataValue.count), unsafeBitCast(-1, to: sqlite3_destructor_type.self))
					}
					guard result == SQLITE_OK else {
						let errmsg = String(cString: sqlite3_errmsg(db))
						throw SQLiteError.queryError("Failed to bind data parameter at index \(index): \(errmsg)")
					}
				case let uuidValue as UUIDv7:
					let result = uuidValue.withUnsafeBytes { bytes in
						sqlite3_bind_blob(statement, paramIndex, bytes.baseAddress, Int32(bytes.count), unsafeBitCast(-1, to: sqlite3_destructor_type.self))
					}
					guard result == SQLITE_OK else {
						let errmsg = String(cString: sqlite3_errmsg(db))
						throw SQLiteError.queryError("Failed to bind UUIDv7 parameter at index \(index): \(errmsg)")
					}
				case nil: fallthrough
				case is NSNull:
					guard sqlite3_bind_null(statement, paramIndex) == SQLITE_OK else {
						let errmsg = String(cString: sqlite3_errmsg(db))
						throw SQLiteError.queryError("Failed to bind null parameter at index \(index): \(errmsg)")
					}
				default:
					throw SQLiteError.queryError("Unsupported parameter type at index \(index): \(type(of: parameter))")
				}
			}
