...
struct CreateTableInterceptionTests {
    ...
    func simpleCreateTableInterception() async throws {
        ...
        let results = try rbdb.query("SELECT name, json(column_names) as column_names_json FROM predicate WHERE name = ?", parameters: ["users"])

        #expect(results.count == 1, "Should have one predicate record")
        #expect(results[0]["name"] as? String == "users", "Table name should be recorded")

        if let columnNamesJson = results[0]["column_names_json"] as? String,
           let columnNamesData = columnNamesJson.data(using: .utf8) {
            let columnNames = try JSONSerialization.jsonObject(with: columnNamesData) as? [String]
            #expect(columnNames == ["id", "name", "email"], "Column names should be parsed correctly")
        } else {
        ...
        #expect(throws: SQLiteError.self) {
            try rbdb.query("INSERT INTO users (id, name, email) VALUES (1, 'Alice', 'alice@example.com')")
        }
    }
    ...
    func createTableIfNotExistsInterception() async throws {
        ...
        try rbdb.query("CREATE TABLE IF NOT EXISTS products (id INTEGER PRIMARY KEY, name TEXT NOT NULL, price REAL)")

        // Check that the predicate was recorded
        let results = try rbdb.query("SELECT name, json(column_names) as column_names_json FROM predicate WHERE name = ?", parameters: ["products"])

        #expect(results.count == 1, "Should have one predicate record")
        #expect(results[0]["name"] as? String == "products", "Table name should be recorded")

        if let columnNamesJson = results[0]["column_names_json"] as? String,
           let columnNamesData = columnNamesJson.data(using: .utf8) {
            let columnNames = try JSONSerialization.jsonObject(with: columnNamesData) as? [String]
            #expect(columnNames == ["id", "name", "price"], "Column names should be parsed correctly")
        } else {
            #expect(Bool(false), "column_names should be accessible as JSON")
        }
    }
    ...
    func createTableWithComplexColumns() async throws {
        ...
        // Check that the predicate was recorded
        let results = try rbdb.query("SELECT name, json(column_names) as column_names_json FROM predicate WHERE name = ?", parameters: ["complex_table"])

        #expect(results.count == 1, "Should have one predicate record")
        #expect(results[0]["name"] as? String == "complex_table", "Table name should be recorded")

        if let columnNamesJson = results[0]["column_names_json"] as? String,
           let columnNamesData = columnNamesJson.data(using: .utf8) {
            let columnNames = try JSONSerialization.jsonObject(with: columnNamesData) as? [String]
            #expect(columnNames == ["id", "name", "created_at", "price", "data"], "Column names should be parsed correctly")
        } else {
            #expect(Bool(false), "column_names should be accessible as JSON")
        }
    }
    ...
    func multipleCreateTableStatements() async throws {
        ...
        let results = try rbdb.query("SELECT name FROM predicate ORDER BY name")

        #expect(results.count == 3, "Should have three predicate records")

        let tableNames = results.compactMap { $0["name"] as? String }
        #expect(tableNames == ["table1", "table2", "table3"], "All table names should be recorded")
    }
    ...
    func createTableWithSpecialTableName() async throws {
        ...
        // Check that the predicate was recorded
        let results = try rbdb.query("SELECT name, json(column_names) as column_names_json FROM predicate WHERE name = ?", parameters: ["user-data"])

        #expect(results.count == 1, "Should have one predicate record")
        #expect(results[0]["name"] as? String == "user-data", "Table name should be recorded without quotes")

        if let columnNamesJson = results[0]["column_names_json"] as? String,
           let columnNamesData = columnNamesJson.data(using: .utf8) {
            let columnNames = try JSONSerialization.jsonObject(with: columnNamesData) as? [String]
            #expect(columnNames == ["id", "info"], "Column names should be parsed correctly")
        }
    }
    ...
    func createTableWithColumnNamesOnly() async throws {
        ...
        // Check that the predicate was recorded
        let results = try rbdb.query("SELECT name, json(column_names) as column_names_json FROM predicate WHERE name = ?", parameters: ["simple_table"])

        #expect(results.count == 1, "Should have one predicate record")
        #expect(results[0]["name"] as? String == "simple_table", "Table name should be recorded")

        if let columnNamesJson = results[0]["column_names_json"] as? String,
           let columnNamesData = columnNamesJson.data(using: .utf8) {
            let columnNames = try JSONSerialization.jsonObject(with: columnNamesData) as? [String]
            #expect(columnNames == ["id", "name", "value"], "Column names should be parsed correctly")
        } else {
            #expect(Bool(false), "column_names should be accessible as JSON")
        }
    }
