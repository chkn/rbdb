...
struct RBDBSchemaTests {
    ...
    func tablesCreatedOnInit() async throws {
        ...
        let results = try rbdb.query("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name")

        let tableNames = results.compactMap { $0["name"] as? String }

        #expect(tableNames.contains("entity"), "entity table should be created")
    ...
    func uuidv7FunctionWorks() async throws {
        ...
        try rbdb.query("INSERT INTO entity (internal_entity_id) VALUES (1)")

        // Query the inserted row
        let results = try rbdb.query("SELECT entity_id FROM entity WHERE internal_entity_id = 1")
