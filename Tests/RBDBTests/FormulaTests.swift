...
@Test func serializePredicate() async throws {
	try assertJSON(Formula.predicate(name: "Foo", arguments: [.string("bar")]), expect: "[\"@Foo\",{\"\":\"bar\"}]")
}
...
@Test func serializeQuantified() async throws {
	let a = Var(id: 0)
	try assertJSON(Formula.quantified(.forAll, a, .predicate(name: "Foo", arguments: [.variable(a)])), expect: "[\"@Foo#\",0,0,[\"@Foo\",{\"v\":0}]]")
}
